...
export default function Page() {
  ...
  useEffect(() => {
    (async () => {
      try {
        ...
        const arr  = json.Allleads || json.leads || json.data || json || [];
    ...
    leads.forEach((l) => {
      const createdById =
        typeof l.createdBy === "string"
          ...
          : null;
      const key = createdById || l.name || "Unknown";

      if (!map[key]) {
        map[key] = {
          id:        key,
          name:      l.name || `Rep ${key.slice(-4)}`,
          branches:  new Set(),
          leads:     0,
          contacted: 0,
          qualified: 0,
      ...
      if (l.stage === "Closed Won") { r.closed++; r.revenue += l.dealValue || 0; }
  ...
  const maxRevenue  = repStats[0]?.revenue || 1;
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4 mb-4">
        <div className="bg-white rounded-xl p-5 shadow-sm border border-slate-100">
          <div className="flex justify-between items-center mb-5">
            ...
            <span className="bg-blue-50 text-blue-500 rounded-full px-3 py-[3px] text-[11px] font-semibold">
              This Month
            </span>
          </div>

          <div className="flex flex-col gap-5">
            {repStats.slice(0, 3).map((rep, i) => {
              const rank = i + 1;
                ...
                <div key={rep.id} className="flex items-start gap-3">
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex justify-between items-center mb-0.5">
                      <span className="font-bold text-[13px]">{rep.name}</span>
                      <div className="flex gap-4 shrink-0">
                        <div className="text-center">
                          <div className="font-extrabold text-[15px]">{rep.closed}</div>
                          <div className="text-[10px] text-slate-400">Closed</div>
                        </div>
                        <div className="text-center">
                          <div className="font-extrabold text-[15px] text-green-500">
                            {formatRevenue(rep.revenue)}
                          </div>
                          <div className="text-[10px] text-slate-400">Revenue</div>
                        </div>
                      </div>
                    </div>
                    <div className="text-[11px] text-slate-400 mb-1.5">{rep.branchLabel}</div>
            ...
            <thead>
              <tr>
                {["REP","BRANCH","LEADS","CONTACTED","QUALIFIED","PROPOSALS","CLOSED","CLOSE RATE","REVENUE","DOCS UPLOADED"].map((h) => (
                  <th
                    key={h}
                    className="text-left px-3 py-2 text-[11px] font-bold text-slate-400 border-b border-slate-100 whitespace-nowrap"
                  >
                    {h}
                  </th>
                ))}
